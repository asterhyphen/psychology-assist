...
class _JournalEntryCardState extends ConsumerState<JournalEntryCard> {
  ...
  Future<void> _summarize() async {
    ...
    try {
      ...
      await ref.read(journalStateProvider.notifier).updateEntry(updatedEntry);
  ...
  void _shareWithTherapist() {
    if (!widget.hasPsychologist) {
      AppSnackBar.showError(
        context,
        title: 'No therapist connected',
        message: 'Connect a care provider before sharing.',
      );
      return;
    }

    final updatedEntry = widget.entry.copyWith(sharedWithPsychologist: true);
    ref.read(journalStateProvider.notifier).updateEntry(updatedEntry);

    AppSnackBar.showSuccess(
      context,
      title: 'Shared with therapist',
      message: 'The summary was shared with ${widget.psychologistEmail}.',
    );
  }
